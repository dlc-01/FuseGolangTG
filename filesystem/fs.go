...
// File represents a file stored in Telegram with associated metadata.
type File struct {
	ID         string
	Name       string
	Size       int64
...
func (d Dir) Remove(ctx context.Context, req *fuse.RemoveRequest) error {
	...
	fullPath := d.Path + "/" + req.Name
	if file, ok := d.FileSystem.Files[fullPath]; ok {
		err := d.FileSystem.TelegramService.DeleteMessage(file.TelegramID)
		if err != nil {
			return err
		}
		delete(d.FileSystem.Files, fullPath)
		d.FileSystem.TelegramService.RemoveMapping(file.TelegramID)
...
func (f FileNode) ReadAll(ctx context.Context) ([]byte, error) {
	data, err := f.FileSystem.TelegramService.FetchFile(f.TelegramID)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func deleteFilesWithTag(fs *FileSystem, tag string) error {
	fs.FilesMutex.Lock()
	defer fs.FilesMutex.Unlock()

	for name, file := range fs.Files {
		if file.Tag == tag {
			err := fs.TelegramService.DeleteMessage(file.TelegramID)
			if err != nil {
				return err
			}
			delete(fs.Files, name)
			fs.TelegramService.RemoveMapping(file.TelegramID)
